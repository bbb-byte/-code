...
| 适用场景 | 防止 archive 导入或补充采样重复执行导致的数据膨胀 |
...
| 字段 | 含义 | 来源 | 说明 |
|:---|:---|:---|:---|
| `unit_price` | 商品单价 | archive 正式数据集 | 正式分析口径直接使用数据集价格字段 |
| `qty` | 购买数量 | 默认值1 | 可扩展支持多件购买场景 |
| `monetary` | 累计消费金额 | 聚合计算 | = Σ(unit_price × qty) |

> **论文说明**：本系统当前以 archive 多品类电商行为数据集中的价格字段计算 RFM-M 值；若未来接入真实订单系统（含 order_id、pay_status、pay_time），可直接使用支付成功金额替换，进一步提高精度。
